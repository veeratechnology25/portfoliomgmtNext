...
export default function CreateLineItemPage({ params }: { params: { id: string } }) {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch existing line items to determine next sequence
      try {
        const lineItemsResponse = await financeAPI.getBudgetLineItems(params.id);
        const existingItems = lineItemsResponse.data;
        const maxSequence = existingItems.length > 0 
          ? Math.max(...existingItems.map((item: any) => item.sequence))
          : 0;
        setNextSequence(maxSequence + 1);
        setFormData(prev => ({ ...prev, sequence: maxSequence + 1 }));
      } catch (error) {
        // No line items exist yet
        setNextSequence(1);
        setFormData(prev => ({ ...prev, sequence: 1 }));
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await financeAPI.createBudgetLineItem(params.id, lineItemData);
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Create Line Item</h1>
                <p className="text-gray-600">
                  Add a new line item to {budget.name} ({budget.code})
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div className="flex items-center">
              ...
              <div>
                <h3 className="text-sm font-medium text-blue-900">Budget Information</h3>
                <p className="text-sm text-blue-700">
                  Total Budget: {parseFloat(budget.total_amount).toLocaleString('en-US', { style: 'currency', currency: 'USD' })} • 
                  Allocated: {parseFloat(budget.allocated_amount).toLocaleString('en-US', { style: 'currency', currency: 'USD' })}
                </p>
              </div>
            </div>
          </div>
