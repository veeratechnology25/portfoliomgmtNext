...
export default function EditSkillPage({ params }: { params: { id: string } }) {
  ...
  const fetchData = async () => {
    try {
      ...
      setFormData({
        ...
        proficiency_levels: skillData.proficiency_levels ? skillData.proficiency_levels.join(', ') : '',
      });
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await organizationAPI.updateSkill(params.id, skillData);
      toast.success('Skill updated successfully');
      router.push(`/organization/skills/${params.id}`);
    } catch (error) {
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <Link href={`/organization/skills/${params.id}`}>
